...
namespace AssetBundleBrowser
{
    ...
    internal class AssetBundleBuildTab
    {
        ...
        private readonly Dictionary<string, bool> _selectBundle = new Dictionary<string, bool>();
        private Vector2 _selectBundlePos = Vector2.zero;
        ...
        internal void OnEnable(EditorWindow parent)
        {
            ...
            _selectBundle.Clear();
            var bundles = AssetBundleModel.Model.DataSource.GetAllAssetBundleNames();
            foreach (var bundleName in bundles)
                _selectBundle.Add(bundleName, false);
        }
        ...
        internal void OnGUI()
        {
            ...
            GUILayout.Label(new GUIContent("Puzzle Game build setup"), centeredStyle);

            // アセットバンドル選択できるようにビルド
            EditorGUILayout.Space();
            GUILayout.Label(new GUIContent("Bundle Lists"), centeredStyle);
            EditorGUILayout.BeginVertical(GUILayout.Height(200));
            _selectBundlePos = EditorGUILayout.BeginScrollView(_selectBundlePos, GUI.skin.box);
            var bundles = AssetBundleModel.Model.DataSource.GetAllAssetBundleNames();
            foreach (var bundleName in bundles)
            {
                var select = EditorGUILayout.ToggleLeft(bundleName, _selectBundle[bundleName]);
                _selectBundle[bundleName] = select;
            }
        ...
        {
            EditorUtility.DisplayProgressBar("Building asset bundles...", "Building", 0);
            ...
            int buildCount = BuildSelectedAssets(buildInfo);
            ...
            EditorUtility.DisplayDialog("Completed!", m_UserData.m_BuildTarget + " Build Complete!\n" + "Total build:" + buildCount, "ok");
        }
        ...
        // ビルド
        int BuildSelectedAssets(ABBuildInfo info)
        {
            ...
            foreach (var bundleName in bundles)
            {
                var select = _selectBundle[bundleName];
                if (!select)
                    continue;

                var build = new AssetBundleBuild();
                var assetNames = AssetBundleModel.Model.DataSource.GetAssetPathsFromAssetBundle(bundleName);
            ...
            // Build all assets
            var manifest = BuildPipeline.BuildAssetBundles(info.outputDirectory, list.ToArray(), info.options, info.buildTarget);
            if (null == manifest)
            {
                Debug.Log("Error build asset bundles");
                return -1;
            }
            foreach (var bundleName in manifest.GetAllAssetBundles())
            {
                info.onBuild?.Invoke(bundleName);
            }
            return list.Count;
        }
    }
